...
async function command(executable: string, args: string[], timeoutMs = 600_000, signal?: AbortSignal): Promise<string> {
  return await new Promise((resolve, reject) => {
    if (signal?.aborted) { reject(new ApiError(503, 'MEDIA_PROCESS_ABORTED', `${executable} was interrupted.`)); return; }
    ...
    const abort = () => { child.kill('SIGKILL'); settle(() => reject(new ApiError(503, 'MEDIA_PROCESS_ABORTED', `${executable} was interrupted.`))); };
    const timer = setTimeout(() => { child.kill('SIGKILL'); settle(() => reject(new ApiError(504, 'MEDIA_PROCESS_TIMEOUT', `${executable} exceeded its time limit.`))); }, timeoutMs);
    signal?.addEventListener('abort', abort, { once: true });
    child.stdout.on('data', (chunk: Buffer) => { stdout += chunk.toString(); });
    child.stderr.on('data', (chunk: Buffer) => { if (stderr.length < 20_000) stderr += chunk.toString(); });
    child.on('error', (error) => { settle(() => reject(new ApiError(500, 'MEDIA_TOOL_UNAVAILABLE', `${executable} is unavailable: ${error.message}`))); });
    child.on('close', (code) => {
      if (code === 0) settle(() => resolve(stdout));
      else settle(() => reject(new ApiError(502, 'MEDIA_PROCESS_FAILED', `${executable} failed with exit code ${code}: ${stderr.slice(-1_000)}`)));
    });
  });
}

export class FfmpegMediaProcessor {
  async probe(path: string, signal?: AbortSignal): Promise<ProbeResult> {
    const output = await command('ffprobe', ['-v', 'error', '-show_format', '-show_streams', '-of', 'json', path], 600_000, signal);
  ...
  async normalize(input: string, output: string, resolution: string, segmentDuration: number, signal?: AbortSignal): Promise<void> {
  ...
  async concatenate(inputs: string[], manifest: string, concatenated: string, signal?: AbortSignal): Promise<void> {
    ...
    await command('ffmpeg', ['-y', '-f', 'concat', '-safe', '0', '-i', manifest, '-c', 'copy', concatenated], 600_000, signal);
  }
